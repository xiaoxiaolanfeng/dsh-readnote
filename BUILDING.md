...
### 4.13 跨节点文本锚点（markdown 渲染会拆碎你的引用）

| 现象 | 原因 | 解法 |
|---|---|---|
| **批注保存成功、刷新后还在，但原文没有高亮**（连气泡也不出现） | 这条批注的引用**横跨了 markdown 的内联格式**。`**加粗**` 渲染成 `<strong>` 后，一句话被拆成好几个文本节点，而定位函数只在**单个文本节点**里 `indexOf` —— 跨格式的引用永远找不到，静默返回 null | 把 root 下所有文本节点按 DOM 顺序串成一条「全局文本」，在其中定位，再把全局偏移映射回 `(节点, 节点内偏移)`。这就是 W3C TextQuoteSelector 的标准实现 |

**触发条件很具体**——用户批注的那句话恰好含加粗：

```
批注 quote: 我花了三轮在源码 clone 里翻找根因，三次全猜错 —— 因为实际运行的根本不是那份源码：
文档源码  : 我花了三轮在**源码 clone** 里翻找根因…因为**实际运行的根本不是那份源码**：
渲染后 DOM: [我花了三轮在][<strong>源码 clone</strong>][ 里翻找根因…][<strong>实际运行的…</strong>][：]
```

**修完的验证**（Playwright 读真实状态，不是看截图猜）：

```js
CSS.highlights.get('readnote-notes')  →  groupSize: 1
                                         firstText: "我花了三轮在源码 clone 里翻找根因…"   ← 含加粗部分，跨节点成功
document.querySelectorAll('.readnote-bubble')  →  1
```

> **文本锚点不能假设「一段引用落在一个 DOM 节点里」。** 只要渲染管线会拆文本
> （加粗、链接、行内代码、KaTeX 公式都会），单节点 `indexOf` 就一定会漏 ——
> 而且是**静默漏**：批注存下来了、界面没报错，只是什么都看不见。

### 4.14 两个「写完了但等于没写」的错

| 现象 | 原因 | 解法 |
|---|---|---|
| **`setRecent(prev => …)` 编译不过** | 我给 `useState` 手写的类型是 `<T>(initial: T) => [T, (next: T) => void]` —— **只支持直接赋值**，不支持函数式更新 | 类型改成 `(next: T \| ((prev: T) => T)) => void`。这是手写 React 运行时类型（不能 `import`）必然要自己兜的坑 |
| **「最近打开」做完了，用户却永远看不到** | 显示条件被我写成 `cwd.length === 0 && recent.length > 0`（只在工作区根显示），可「返回列表」回到的是**文档所在目录** —— 条件永远不成立 | 去掉目录条件，每个目录都显示最近列表 |

**顺带验证了 `noEmitOnError` 的价值**（见 4.8）：第一个错误发生时编译失败 → **产物没被写坏** →
测试跑的还是上一个可用版本（没崩，只是新功能没生效）。这正是当初加它的目的。

> **「功能写完了」和「用户能用到」是两件事。** 第二个 bug 代码本身没错、DOM 里也真渲染了分节，
> 只是**那个分支永远不会被走到**。这类错只能靠"真去点一遍"发现 —— 又一次。

### 4.15 会话消息流里混着「不是对话」的东西

| 现象 | 原因 | 解法 |
|---|---|---|
| **对话栏里冒出一条以 `<system-reminder>` 开头的消息** | `session.deriveMessages()` 返回的是**模型看到的全部输入**，其中包含宿主注入的环境上下文（AGENTS.md 提醒之类）。它的 `role` 是 `user`，所以按 role 过滤拦不住 | 按内容前缀过滤掉 `<system-reminder` 开头的条目 |

**排查时的一个小陷阱**：过滤上线后消息总数**还是 24**，看着像没生效 ——
其实是因为上限本来就是「最后 24 条」，删掉一条自然会往前补一条。
**判断过滤是否生效要看内容，不能看数量。**

> **「会话里的消息」≠「用户在对话框里说的话」。** 会话日志是模型视角的完整输入，
> 展示给用户时必须按来源过滤 —— 否则用户会看到一堆自己从没发过的"自己的话"。
